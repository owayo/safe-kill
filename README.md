...
- **Terminal Output Sanitization**: ANSI escape sequences, newlines, and other control characters in process names, argv, and error message bodies are escaped (`\xHH`) before being printed by `--list`, kill result lines (both `name` and `message`), and the stderr error path (`safe-kill: ...`). Blocks display spoofing attacks via crafted argv even when the kill is refused (e.g. `NotDescendant` / `Denylisted` errors that embed the offending process name).
...
11. **Terminal Output Sanitization**: Process names, command-line arguments, and error message bodies fetched from the OS are escaped (`\n`, `\r`, `\t`, `\xHH`) on every output path: `--list`, kill result lines (both the `name` column and the `message` column, since `KillResult::failure` stores `error.to_string()` which can embed the offending process name via `NotDescendant(pid, name)` / `Denylisted(name)`), and the stderr error line emitted by `main()` (`safe-kill: ...`). A hostile process started with crafted argv containing ANSI escape sequences (screen clear, cursor moves, OSC sequences) cannot rewrite or hide rows of the output even when its kill is refused, and the escape sequence cannot be cut mid-byte by the column-width truncation logic.
...
- **Library Unit Tests**: 396 tests covering all modules
