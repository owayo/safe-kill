...
- **Terminal Output Sanitization**: ANSI escape sequences, newlines, and other control characters in process names/argv are escaped (`\xHH`) before being printed by `--list` and kill result lines, blocking display spoofing attacks via crafted argv
...
11. **Terminal Output Sanitization**: Process names and command-line arguments fetched from the OS are escaped (`\n`, `\r`, `\t`, `\xHH`) before being printed by `--list` or kill-result lines. A hostile process started with crafted argv containing ANSI escape sequences (screen clear, cursor moves, OSC sequences) cannot rewrite or hide rows of the output, and the escape sequence cannot be cut mid-byte by the column-width truncation logic.
...
- **Library Unit Tests**: 369 tests covering all modules
