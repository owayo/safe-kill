...
- `safe-kill --port 3000`: ポート3000が `allowed_ports` に設定されていれば、自己破壊防止・拒否リスト・root PID 保護を維持したまま、**親子関係に関係なく**終了可能。別ターミナルで起動したままの開発サーバー等を終了する場合に便利。
...
- **ライブラリユニットテスト**: 全モジュールを網羅する308テスト
