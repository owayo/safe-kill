...
| 変数 | 説明 |
|-----|------|
| `SAFE_KILL_ROOT_PID` | 親子関係チェックのルートPIDを上書き（`0` や無効値は無視） |
...
- **ライブラリユニットテスト**: 全モジュールを網羅する286テスト
- **バイナリユニットテスト**: CLI出力ユーティリティとバージョン検証の23テスト
- **統合テスト**: 実際のプロセスツリーを使用した48テスト
- **E2Eテスト**: CLI動作を検証する65テスト
