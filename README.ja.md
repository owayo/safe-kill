...
| `--port <PORT>` | `-p` | 指定ポートを使う設定済み TCP リスナーまたは UDP ソケットを終了 |
...
`--port` は TCP では `LISTEN` 状態のソケットだけを対象にします。同じローカルポートを持つ接続済み TCP クライアントソケットは対象外です。UDP は接続状態を持たないため、ローカルポート一致で対象にします。
...
- `safe-kill --port 3000`: ポート3000が `allowed_ports` に設定されていれば、自己破壊防止・拒否リスト・root PID 保護を維持したまま、**親子関係に関係なく** TCP リスナーまたは UDP ソケットを終了可能。別ターミナルで起動したままの開発サーバー等を終了する場合に便利。
...
- **ライブラリユニットテスト**: 全モジュールを網羅する314テスト
- **バイナリユニットテスト**: CLI出力ユーティリティとバージョン検証の26テスト
- **統合テスト**: 実際のプロセスツリーを使用した77テスト
- **E2Eテスト**: CLI動作を検証する80テスト
