...
- **端末制御文字のサニタイズ**: OS から取得したプロセス名・コマンドライン引数に含まれる ANSI escape・改行・制御文字を `\xHH` 等にエスケープしてから `--list` / kill 結果を表示。悪意ある起動引数による表示行の上書きや端末状態の改ざんを防ぐ
...
11. **端末制御文字のサニタイズ**: OS から取得したプロセス名・コマンドライン引数に含まれる ANSI escape（画面消去・OSC 等）、改行、タブ、その他制御文字を `\xHH` 等のエスケープ表記に置き換えてから `--list` / kill 結果を出力する（`main.rs::sanitize_terminal`）。任意の引数で起動されたプロセスでも表示行の上書きや端末状態の改ざんを防ぐ。サニタイズは `truncate` より前に行うため、escape sequence の途中で切られて端末状態が残ることもない
